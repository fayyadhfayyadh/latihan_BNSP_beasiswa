...
<div class="container mt-4">
    ...
    <form action="{{ route('beasiswa.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            <label>IPK</label>
            <input type="text" name="ipk" value="{{ $randomIpk }}" class="form-control" readonly>
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="jenis_id" class="form-control" required>
                ...
                @foreach($jenis as $j)
                    <option value="{{ $j->id }}">{{ $j->nama_beasiswa }}</option>
                @endforeach
            </select>
        </div>

        <div class="mb-3">
            <label>Upload Berkas (PDF/JPG/PNG)</label>
            <input type="file" name="berkas" class="form-control" accept=".pdf,.jpg,.png" required>
        </div>
